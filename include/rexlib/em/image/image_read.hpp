 ...
 * @ref image_location::get_position_in_stack names within its file, or the
 ...
 * the caller already has rather than allocating one. Neither
 * @ref completion::wait nor @ref completion::get of the completion returned
 * may be called from within a task already running on the executor @p source
 ...
 * those carrying a position with those carrying none.
 */
REXLIB_API
 ...
 * slice @ref image_location::get_position_in_stack indexes within the file
 * or the whole file when it carries no position.
 *
 * @par Positions
 ...
 * an odd extent and the origin a Fourier transform of the patch would use
 * for an even one.
 *
 * @par Borders
 ...
 * @p positions does not hold one position per slot of @p destination, or if
 * a position does not have the rank of one patch.
 */
REXLIB_API
