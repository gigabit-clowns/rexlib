 ...
 * 
 */
REX_CONSTEXPR auto dynamic_extent = std::numeric_limits<std::size_t>::max(); 
...
{
	T* data;
	static REX_CONST_CONSTEXPR std::size_t size = S;

	span_storage() = delete;
	REX_CONSTEXPR span_storage(T* ptr, std::size_t count) noexcept;
...
struct span_storage<T, dynamic_extent> 
{
	...
	REX_CONSTEXPR span_storage() noexcept;
	REX_CONSTEXPR span_storage(T* ptr, std::size_t count) noexcept;
	span_storage(const span_storage &other) = default;
	span_storage(span_storage &&other) = default;
	~span_storage() = default;
...
{
	static REX_CONST_CONSTEXPR T* data = nullptr;
	static REX_CONST_CONSTEXPR std::size_t size = 0;
...
public:
	 ...
	 */
	REX_CONSTEXPR span(pointer ptr, size_type count);
	 ...
	 */
	REX_CONSTEXPR span(pointer first, pointer last);
	...
	REX_CONSTEXPR span(const span<value_type, Extent2> &other);
	 ...
	 */
	template <std::size_t Extent2>
	REX_CONSTEXPR span(const span<const value_type, Extent2> &other);
	 ...
	 */
	REX_CONSTEXPR size_type size() const noexcept;
	 ...
	 */
	REX_CONSTEXPR size_type size_bytes() const noexcept;
	 ...
	 */
	REX_NODISCARD REX_CONSTEXPR bool empty() const noexcept;
	 ...
	 */
	REX_CONSTEXPR reference operator[](size_type idx) const noexcept;
	 ...
	 */
	REX_CONSTEXPR reference front() const noexcept;
	 ...
	 * when empty() == true leads to undefined behavior.
	 */
	REX_CONSTEXPR reference back() const noexcept;
	 ...
	 */
	REX_CONSTEXPR pointer data() const noexcept;
	 ...
	 */
	REX_CONSTEXPR iterator begin() const noexcept;
	 ...
	 */
	REX_CONSTEXPR iterator end() const noexcept;
	 ...
	 */
	REX_CONSTEXPR reverse_iterator rbegin() const noexcept;
	 ...
	 */
	REX_CONSTEXPR reverse_iterator rend() const noexcept;
...
REX_CONSTEXPR span<byte, N*sizeof(T)> 
as_bytes(const span<T, N> &other) noexcept;

template <typename T, std::size_t N>
REX_CONSTEXPR span<const byte, N*sizeof(T)> 
...
template <typename T, std::size_t N>
REX_CONSTEXPR span<T, N> make_span(std::array<T, N> &array) noexcept;

template <typename T, std::size_t N>
REX_CONSTEXPR span<const T, N> 
make_span(const std::array<T, N> &array) noexcept;

template <typename T>
REX_CONSTEXPR span<T> make_span(std::vector<T> &array) noexcept;

template <typename T>
REX_CONSTEXPR span<const T> 
