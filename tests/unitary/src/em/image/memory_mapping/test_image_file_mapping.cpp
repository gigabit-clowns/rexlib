	...
	"[image_file_mapping]" )
{
	...
	SECTION( "the whole file is mapped from its first byte" )
	{
		...
		const image_file_mapping mapping(path.get(), read_only);
	...
	"[image_file_mapping]" )
{
	...
	SECTION( "what is written reaches the storage" )
	{
		...
		{
			image_file_mapping mapping(path.get(), read_write);
	...
	SECTION( "a second mapping sees what the first one wrote" )
	{
		...
		image_file_mapping writer(path.get(), read_write);
		writer.get_data()[0] = as_byte(0x42);
		writer.flush();

		const image_file_mapping reader(path.get(), read_only);
	...
	"[image_file_mapping]" )
{
	...
	{
		image_file_mapping original(path.get(), read_only);
	...
	SECTION( "a mapping assigned over keeps reading the file" )
	{
		...
		image_file_mapping original(path.get(), read_only);
		image_file_mapping target(other.get(), read_only);
