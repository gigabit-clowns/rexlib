...
{
	REXLIB_ASSERT( extents.size() < target_size );
...
)
{
	...
	REXLIB_ASSERT( extents1.size() == extents2.size() );
...
)
{
	...
	REXLIB_ASSERT( result.size() >= other.size() );
...
) noexcept
{
	...
	REXLIB_ASSERT( padding + n == extents1.size() );
...
) noexcept
{
	...
	REXLIB_ASSERT( to_extents.size() >= from_extents.size() );
	const auto padding = to_extents.size() - from_extents.size();
	const auto n = from_extents.size();
	REXLIB_ASSERT( padding + n == to_extents.size() );
